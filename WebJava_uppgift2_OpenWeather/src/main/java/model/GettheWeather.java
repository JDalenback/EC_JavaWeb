...
public class GettheWeather {
	...
	public static void getWeather(weatherBean wBean) throws IOException {
		...
		String URLtoSend = "http://api.openweathermap.org/data/2.5/weather?q=" + wBean.getCityStr() + ","
				+ wBean.getCountryStr() + "&APPID=e7ecba88b560bec253ebe5232e031b24&mode=xml";
		...
		String elementValue = element.getAttribute("name");
		wBean.setCloudsStr(elementValue);
		
		nList = doc.getElementsByTagName("temperature");
		element = (Element) nList.item(0);
		elementValue = element.getAttribute("value");
		wBean.setAvgTemperature(Double.parseDouble(elementValue) - 273.15);
		
		wBean.setDate(java.time.LocalDate.now().toString());
	}
